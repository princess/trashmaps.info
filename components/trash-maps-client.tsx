...
function TrashMapsContent() {
  ...
  useEffect(() => {
    if (query) {
      const fetchQueryLocation = async () => {
        try {
          const res = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(query)}&format=json&limit=1`);
          if (res.ok) {
            const data = await res.json();
            if (data && data[0]) {
              handleSelectLocation(parseFloat(data[0].lat), parseFloat(data[0].lon));
              setShowWelcomeModal(false); // Hide modal if searching
            }
          }
        } catch (e) {
          console.error("Error fetching query location:", e);
        }
      };
      fetchQueryLocation();
    }
  }, [query, handleSelectLocation]);

  const handleGeolocation = useCallback(() => {
    if (query) return; // Don't auto-locate if a query is present
    setIsLocating(true);
          ...
          <DialogHeader>
            <div className="flex items-center justify-center gap-2 mb-2 sm:mb-4">
              <div className="bg-green-600 p-2 sm:p-3 rounded-lg">
                <Recycle className="h-6 w-6 sm:h-8 text-white" />
              </div>
...
export default function TrashMapsClient() {
  return (
    <Suspense fallback={
        <div className="h-full w-full flex items-center justify-center bg-gray-50">
            <Recycle className="h-12 w-12 text-green-600 animate-spin" />
        </div>
    }>
      <TrashMapsContent />
    </Suspense>
